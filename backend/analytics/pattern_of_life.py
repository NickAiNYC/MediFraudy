...
def detect_kickback_patterns(
    ...
    if risk_score >= 40:
        indicator = KickbackIndicator(
            ...
            enrollment_spike_dates=[s["month"] for s in spikes] if spikes else [],
            risk_score=risk_score,
...
class ElitePatternOfLifeAnalyzer:
    """Class wrapper for Pattern of Life analysis."""
    
    def __init__(self, db: Session):
        self.db = db

    def analyze_provider(self, provider_id: int, lookback_days: int = 365) -> Dict[str, Any]:
        """Run comprehensive analysis on a provider."""
        return comprehensive_pattern_analysis(self.db, provider_id, lookback_days)
