...
def _get_top_providers(db: Session, limit: int = 50) -> List[int]:
    """Get IDs of top providers by claim volume."""
    return [id for id, in db.query(Provider.id)
            .join(Claim)
...
def detect_fraud_patterns(
    ...
    days_back: int = 365 * 1,  # Reduced default to 1 year for performance
    ...
    Args:
        ...
        limit: If provider_id is None, limit scan to top N providers by volume.
    ...
    """
    # Optimization: If no specific provider and limit set, scan top providers individually
    if provider_id is None and limit:
        # Default limit to 10 if not specified to avoid timeouts
        scan_limit = min(limit, 15)
        top_ids = _get_top_providers(db, scan_limit)
        all_results = []
        logger.info(f"Scanning top {scan_limit} providers for fraud patterns...")
        for pid in top_ids:
            try:
                # Recursive call for each provider
                provider_results = detect_fraud_patterns(
                    db, 
                    provider_id=pid, 
                    limit=None, 
                    days_back=days_back, 
                ...
                # Stop if we have enough patterns to show
                if len(all_results) > 100:
                    break
            except Exception as e:
                logger.error(f"Error scanning provider {pid}: {e}")
                continue
        return all_results
...
def _detect_high_volume_billing(
    db: Session, 
    provider_id: Optional[int] = None,
    days_back: int = 365, # Default to 1 year
) -> List[Dict[str, Any]]:
    ...
    """
    # Use shorter lookback for global scan to improve speed
    actual_days = days_back if provider_id else min(days_back, 180)
    cutoff_date = datetime.now() - timedelta(days=actual_days)
    ...
    if provider_id:
        ...
        # Limit to top providers if global scan
...
def _detect_unusual_code_combinations(
    ...
    if provider_id:
        ...
        # If scanning all, limit to top providers to avoid massive joins
