...
def iter_detect_and_load(
    file_path: str,
    chunk_size: int = 50000,
) -> Generator[pd.DataFrame, None, None]:
    """Auto-detect format and yield data in chunks.
    ...
    file_size = os.path.getsize(file_path) / (1024**3)
    logger.info(f"Streaming dataset from {file_path} ({file_size:.2f} GB)")
    ...
    if file_path.endswith(".zip"):
        with zipfile.ZipFile(file_path, "r") as zip_ref:
            ...
            logger.info("Streaming %s from archive", data_files[0])
            with zip_ref.open(data_files[0]) as f:
                if data_files[0].endswith(".parquet"):
                    # Parquet doesn't support streaming from zip stream easily with pandas
                    # We might need to extract it or use pyarrow with memory mapping
                    # For now, let's assume CSV for large zipped files or use a temp file
                    raise NotImplementedError("Streaming Parquet from Zip not fully supported yet")
                else:
                    for chunk in pd.read_csv(f, chunksize=chunk_size, low_memory=False):
                        yield chunk
    
    elif file_path.endswith(".parquet"):
        yield from load_parquet_chunks(file_path, row_group_size=chunk_size)
        
    else:
        # Assume CSV
        yield from load_csv_chunks(file_path, chunk_size=chunk_size)
...
class MedicaidDataLoader:
    ...
    def iter_data(
        ...
        """Yield data in chunks for memory-efficient processing.
        ...
        if not os.path.exists(self.file_path):
            ...
            self.ensure_data(sample=1) # Trigger download verification logic
        
        logger.info(f"Iterating data from {self.file_path}...")
        yield from iter_detect_and_load(self.file_path, chunk_size=chunk_size)
