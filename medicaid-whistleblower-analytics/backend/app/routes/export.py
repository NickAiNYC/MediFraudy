...
# Configuration
EXPORT_DIR = Path("/app/exports")  # Docker path
...
async def export_case_package(
    ...
    Args:
        ...
        format: "json" or "pdf" output format
        include_pol: Whether to include fresh POL analysis
    
    Returns:
        JSON package or PDF file download
    """
    # Get case
    ...
    if format == "pdf":
        ...
        pdf_path = await _generate_pdf(package, case.id)
        return FileResponse(
            path=pdf_path,
            filename=f"case_{case.id}_export.pdf",
...
async def _generate_pdf(package: ExportPackage, case_id: int) -> str:
